    ...
    try {
        ...
        if (brand_name) {
            if (brand_name.length < 2) {
                return res.status(400).send({ message: "Brand name must be at least 2 characters long" });
            }
        ...
        if (!changesMade) {
            return res.status(400).send({ message: "No updates provided or no changes detected" });
        }
